...
int lossy_coding::erroEnc (int valorPixel, int valorPrevisto){
...
int lossy_coding::ValorPixelDec (int erro, int valorPrevisto,int quantization){
    ...
    int retu = residual <<quantization;
    return retu;
}
...
void lossy_coding::preditor_JPEG_LS (Mat matriz,Mat &erMat, Mat &prev, int quantization){
    ...
    for (int i=0;i<(matriz.size().height);++i){ //row
        for(int j=0;j<(matriz.size().width);++j){ //columns
            ...
            //introduzir erro
            int erro = auxerro + ((auxerro>>quantization)<<quantization);
...
void lossy_coding::golombEnc(int erro,int m){
...
void lossy_coding::golombDesc(Mat prevY, Mat prevV, Mat prevU, Mat erroY, Mat erroV, Mat erroU, Mat &ValorY, Mat &ValorV, Mat &ValorU, int quantization){
    //ler de um ficheiro 
    for (int i=0;i<(erroY.size().height)-1;++i){ //row
        for(int j=0;j<(erroY.size().width);++j){ //columns
            ...
            ValorY.at<int>(i,j) = ValorPixelDec(code, prevY.at<int>(i,j),quantization);
    ...
    for (int i=0;i<(erroV.size().height)-1;++i){ //row
        for(int j=0;j<(erroV.size().width);++j){ //columns
            //ler para o code
            int code=0; //vem do descodificador de golomb
            ValorV.at<int>(i,j)  = ValorPixelDec(code, prevY.at<int>(i,j),quantization);
        }
    }
    ...
    for (int i=0;i<(erroU.size().height)-1;++i){ //row
        for(int j=0;j<(erroU.size().width);++j){ //columns
            //ler para o code
            int code=0; //vem do descodificador de golomb
            ValorU.at<int>(i,j) = ValorPixelDec(code, prevY.at<int>(i,j),quantization);
        }
    }    
}
...
int main(int argc,char *argv[]) {
    ...
    //matriz com os erros todos de y com erro introduzido (<<quantization)
    ...
    lossy.golombDesc(prevY,prevV,prevU,erroY,erroV,erroU,ValorY,ValorV,ValorU,quantization);
    ...
    //int erpijorg = lossy.erroEnc (986452865,7);
    //int ver = lossy.ValorPixelDec(erpijorg,7);
    //printf("ola-< %d",ver);
